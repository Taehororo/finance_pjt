...
from .serializers import SavingProductsBaseInfoSerializer, SavingProductsOptionSerializer
...
def get_saving_products(request):
    ...
    for base_data in baseList:
        SavingProductsBaseInfo.objects.create(
    ...
    for option_data in optionList:
            base_instance = SavingProductsBaseInfo.objects.get(fin_prdt_cd=option_data['fin_prdt_cd'])
            SavingProductsOption.objects.create(
...
def product_list(request):
    # 모든 적금 상품 목록을 12개월 기준 최고우대금리 내림차순 정렬로 반환
    ...
    # 데이터베이스에서 모든 적금 상품을 가져옴
    products = get_list_or_404(SavingProductsBaseInfo)
    ...
    #         'has_rate': True
    #         })
    #     except SavingProductsOption.DoesNotExist:
    ...
    # serializer = SavingProductsBaseInfoSerializer(sorted_products_only, many=True)
